...
def render_all_views(
	...
	for i, image_name in enumerate(tqdm(parser.image_names, desc="Rendering")):
		...
		if raycaster is not None:
			...
			# valid_fg = fgmask > 0
			# if valid_fg.any():
			# 	global_min_z = min_z[valid_fg].min()
			# 	global_max_z = max_z[valid_fg].max()

			# 	ipmask_condition = (alpha < 0.9) & valid_fg & (depth >= global_min_z) & (depth <= global_max_z)
			# else:
			# 	ipmask_condition = np.zeros_like(fgmask, dtype=bool)

			# ipmask = ipmask_condition.astype(np.uint8)

			ipmask = ((alpha < 0.9) & (fgmask > 0) & (depth >= min_z) & (depth <= max_z)).astype(np.uint8)
			
			# 1. Filter out very small noise bits
			contours, _ = cv2.findContours(ipmask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
			filtered_ipmask = np.zeros_like(ipmask)
			for cnt in contours:
				if cv2.contourArea(cnt) > 20:  # Drop fragments <= 20 pixels
					cv2.drawContours(filtered_ipmask, [cnt], -1, 1, thickness=cv2.FILLED)
			
			# 2. Iteratively dilate the remaining parts bounded by fgmask
			kernel = np.ones((5, 5), np.uint8)
			curr_mask = filtered_ipmask
			max_iters = 50  # Prevent infinite loop just in case
			for _ in range(max_iters):
				prev_sum = curr_mask.sum()
				dilated = cv2.dilate(curr_mask, kernel, iterations=1)
				curr_mask = cv2.bitwise_and(dilated, fgmask) # Bound by fgmask
				if curr_mask.sum() == prev_sum: # Converged
					break

			# 3. Compute convex hull of the expanded pieces
			contours, _ = cv2.findContours(curr_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
			hull_ipmask = np.zeros_like(ipmask)
			if len(contours) > 0:
				all_pts = np.vstack(contours)
				hull = cv2.convexHull(all_pts)
				cv2.fillPoly(hull_ipmask, [hull], 1)

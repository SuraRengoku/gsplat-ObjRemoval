...
def recover_points_inside_mesh_to_foreground(
    ...
    if save_path is not None:
        try:
            ...
            print(f"  Failed to save mesh to {save_path}: {e}")
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.03,
        help="(Used with --mesh_recover) alpha-shape parameter, or voxel_size when using sdf method."
    )
    parser.add_argument(
        "--mesh_scale",
        type=float,
    ...
    parser.add_argument(
        "--mesh_save_ply",
        action="store_true",
        help="Export the watertight mesh generated by --mesh_recover to a .ply file."
    )
    parser.add_argument(
        "--export_split_surface",
        action="store_true",
    ...
    if args.mesh_recover:
        mesh_save_path = None
        if getattr(args, "mesh_save_ply", False):
            fg_output_path, _ = resolve_output_paths(args.ckpt, args.output, args.lthreshold, args.rthreshold)
            mesh_save_path = str(fg_output_path.with_name(f"{fg_output_path.stem}_mesh.ply"))
